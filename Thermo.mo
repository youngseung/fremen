  ...
  model Test_rho "test case for density" 
    ...
    model Test_h "test case for enthalpy" 
      ...
      function approximatederivative_h 
          ...
          output Real der_T "The rate of variation of temperature.";
    ...
    equation 
          H[1] =  h_ch3oh_gas(T) - h_ch3oh_gas(T_ref)  + dhf(Methanol,GasPhase);
      
          H[2] =  h_ch3oh_liq(T) - h_ch3oh_liq(T_ref) + dhf(Methanol,LiquidPhase);
      
          H[3] =  h_h2o_gas(T) - h_h2o_gas(T_ref) + dhf(Water,GasPhase);
      
          H[4] =  ShomateEnthalpy(T, ShomateH2O) - ShomateEnthalpy(T_ref, ShomateH2O) + dhf(Water,LiquidPhase);
      
          H[5] =  ShomateEnthalpy(T, ShomateO2) - ShomateEnthalpy(T_ref, ShomateO2);
      
          H[6] =  ShomateEnthalpy(T, ShomateCO2) - ShomateEnthalpy(T_ref, ShomateCO2) + dhf(CarbonDioxide,GasPhase);
      
          H[7] =  ShomateEnthalpy(T, ShomateN2) - ShomateEnthalpy(T_ref, ShomateN2);
      
         derivative_error_h = approximatederivative_h(T,n,p,dhf,H,der_H) - der_T(T,n,p);
...
model Test_D \"test case for diffusion coeficients\"
      ...
      Diffusion coefficient Dij := pDoverT175(i,j) * T^1.75 / p;
