...
<p>The generic interface of an electrode (be it anode or cathode).
    ...
    Units.MolarFlow[Thermo.Species] r
  ...
  equation
    ...
    if EtaIsFrozen then
      der(eta)         = 0;
      r[Species.Methanol]      = -i/6/F;
      r[Species.Water]         = -i/6/F;
      r[Species.CarbonDioxide] = i/6/F;
    else
      C * der(eta)     = i/A - ir;
      r[Species.Methanol]      = -r1*A;
      r[Species.Water]         = -r2*A;
      r[Species.CarbonDioxide] = r3*A;
    end if;
    r[Species.Oxygen]   = 0;
    r[Species.Nitrogen] = 0;
